...
function showImages(type) {
    ...
    var gallery = document.getElementById("gallery_products");
        ...
        if (im <= numberOfImages) {
            tmpString = tmpString + "<a class=\"fancybox-buttons\" data-fancybox-group=\"button\" title = \"Product ID: "+type+im+"\" href=\"images/products/" + type + "/" + im + ".jpg\"><img src= \"images/products/" + type + "/preview/" + im + ".jpg\" alt = \"\" /></a >";
            currentNumber = im;
        } 
        else {
...
function translateBreadCrumbAndSetActiveButton(langSN) {
    var type, subtype;
    var activeButtonNumber;
    if (langSN == 2) { //if we switch from French
